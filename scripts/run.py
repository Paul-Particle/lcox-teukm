"""
run.py — entry point: render the `fleet` study (assumptions.yaml + studies.yaml) into the baseline
results artifact.

The fleet sweep is a study like any other (all cases, the op_v_kn lever + d_km condition, no
sampling), evaluated through the same `ingest` -> `evaluate_design` path as every study. Each
...
from common.paths import ASSUMPTIONS_PATH, STUDIES_PATH, RESULTS_DIR, LCOT_PARQUET, LCOT_CSV
